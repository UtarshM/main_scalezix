  ...
  return (
    <main className="section-shell py-20 md:py-24">
      ...
      <div className="max-w-5xl">
        ...
        <p className="mt-5 max-w-4xl text-base leading-8 text-slate-300">{service.overview}</p>
      </div>

      <section className="mt-12 grid gap-5 lg:grid-cols-[1.05fr_0.95fr]">
        <div className="mesh-card rounded-[2rem] p-6 md:p-8">
          <h2 className="text-2xl font-medium text-white">Why teams buy this service</h2>
          <p className="mt-5 text-base leading-8 text-slate-300">{service.problemStatement}</p>
          <p className="mt-5 text-base leading-8 text-slate-300">{service.solutionAngle}</p>
        </div>

        <div className="metal-panel rounded-[2rem] p-6 md:p-8">
          <h2 className="text-2xl font-medium text-white">Expected outcomes</h2>
          <div className="mt-6 grid gap-3">
            {service.outcomes.map((outcome) => (
              <div key={outcome} className="rounded-[1.2rem] border border-white/8 bg-white/[0.02] p-4">
      ...
      <section className="mt-10 grid gap-5 lg:grid-cols-2">
        ...
        <div className="mesh-card rounded-[2rem] p-6 md:p-8">
          <h2 className="text-2xl font-medium text-white">Who this is for</h2>
          <div className="mt-6 space-y-3">
            {service.audience.map((audience) => (
              <div key={audience} className="rounded-[1.2rem] border border-white/8 bg-white/[0.02] p-4">
                <p className="text-sm leading-7 text-slate-200">{audience}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mt-10 grid gap-5 lg:grid-cols-[0.95fr_1.05fr]">
        ...
        <div className="metal-panel rounded-[2rem] p-6 md:p-8">
          <h2 className="text-2xl font-medium text-white">Answer-engine friendly basics</h2>
          <div className="mt-6 space-y-4">
      ...
      <section className="mt-10 grid gap-5 lg:grid-cols-2">
        <div className="metal-panel rounded-[2rem] p-6 md:p-8">
          <h2 className="text-2xl font-medium text-white">Featured proof angle</h2>
          <div className="mt-6 space-y-3">
          ...
          {relatedCaseStudy ? (
            <Link
              ...
              className="mt-6 inline-flex items-center text-sm text-[#9ab9ff] transition hover:text-white"
            >
        ...
        <div className="mesh-card rounded-[2rem] p-6 md:p-8">
          <h2 className="text-2xl font-medium text-white">Supporting insights</h2>
          <div className="mt-6 space-y-3">
            {relatedPosts.map((post) => (
              ...
              >
                <p className="text-xs uppercase tracking-[0.16em] text-slate-400">{post.category}</p>
                <p className="mt-2 text-sm font-medium leading-6 text-white">{post.title}</p>
              </Link>
            ))}
          </div>
        </div>
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {featuredCities.map(({ city, href }) => (
            ...
            >
              ...
              <h3 className="mt-3 text-2xl font-medium text-white">{service.name} in {city.name}</h3>
