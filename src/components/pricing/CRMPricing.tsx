...
export function CRMPricing({ showTitle = true, className }: CRMPricingProps) {
  const [billingCycle, setBillingCycle] = useState<"monthly" | "annual">("monthly");
  const [selectedPlanId, setSelectedPlanId] = useState<string>("crm-growth");
  const [selectedAddons, setSelectedAddons] = useState<{ [key: string]: boolean }>({
    "ai-voice-notes": false,
    "tally-erp-sync": false,
  });
  ...
  const selectedPlan =
    crmPricingPlans.find((p) => p.id === selectedPlanId) || crmPricingPlans[1];
  ...
  const addonsTotalNum = crmAddOns.reduce((acc, addon) => {
  ...
  const getCtaLink = (planName: string, total: number) => {
    const activeAddonsList = crmAddOns
    ...
    const text = encodeURIComponent(
      `Hi Scalezix! I'm interested in the Scalezix CRM ${planName} plan (₹${total.toLocaleString("en-IN")}/mo${addonsText}). Please share deployment details.`
    );
  ...
  return (
    <section className={cn("w-full max-w-6xl mx-auto flex flex-col gap-14", className)}>
      ...
      {showTitle && (
        <div className="text-center max-w-3xl mx-auto flex flex-col items-center gap-4">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-[#473BFD]/20 bg-[#473BFD]/10 text-xs font-mono font-semibold text-[#473BFD] dark:text-[#D3FDB1]">
            ...
            SCALEZIX SALES AUTOMATION & CRM PRICING
          </div>

          <h2 className="text-3xl md:text-5xl font-bold tracking-tight text-slate-900 dark:text-white leading-[1.12]">
            High-Velocity Sales CRM. <br />
            <span className="text-[#473BFD] dark:text-[#D3FDB1]">Zero Per-User Seat Taxes.</span>
          </h2>

          <p className="text-sm md:text-base text-slate-600 dark:text-slate-300 leading-relaxed max-w-2xl">
            Stop paying per-seat penalties for generic software. Scalezix CRM centralizes your pipeline, connects natively to WhatsApp & ERPs, and uses AI to qualify deals faster.
          </p>
      ...
      {/* 3 Main CRM Pricing Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
        {crmPricingPlans.map((plan) => {
              ...
              <div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <BarChart3 className="h-5 w-5 text-[#473BFD] dark:text-[#D3FDB1]" />
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-white/10 pb-6">
          <div>
            <div className="flex items-center gap-2 text-xs font-mono font-bold uppercase tracking-wider text-[#473BFD] dark:text-[#D3FDB1]">
              <Sparkles className="h-3.5 w-3.5" />
              PLUG-AND-PLAY CRM UPGRADES
            </div>
            <h3 className="text-2xl font-bold text-slate-900 dark:text-white mt-1">
              Supercharge Pipeline Velocity
            </h3>
            <p className="text-xs text-slate-600 dark:text-slate-400 mt-1 max-w-xl">
              Attach advanced voice logging or GST accounting automation with zero configuration downtime.
            </p>
          </div>
          ...
          <table className="w-full text-xs text-left">
            <thead>
              <tr className="border-b border-slate-200 dark:border-white/10 text-slate-500 dark:text-slate-400">
                ...
                <th className="py-3 px-4 font-semibold text-center text-[#473BFD] dark:text-[#D3FDB1]">Growth</th>
                <th className="py-3 px-4 font-semibold text-center">Enterprise</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              {crmFeatureComparison.map((row, idx) => (
                <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-white/5">
                  ...
                  <td className="py-3 px-4 text-center">
                    {typeof row.starter === "boolean" ? (
                      ...
                      <span className="text-slate-600 dark:text-slate-300 font-mono">{row.starter}</span>
                    )}
                  </td>
                  <td className="py-3 px-4 text-center font-semibold text-[#473BFD] dark:text-[#D3FDB1]">
                    {typeof row.growth === "boolean" ? (
                      ...
                      <span className="font-mono">{row.growth}</span>
                    )}
                  </td>
                  <td className="py-3 px-4 text-center">
      ...
      <div className="rounded-3xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-card/20 p-8 md:p-10">
        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-6">
          Frequently Asked Questions about Scalezix CRM
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs leading-relaxed">
          {crmPricingFaqs.map((faq, fidx) => (
